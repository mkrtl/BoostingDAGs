...
class BaseDAGBooster:
    ...
    def __init__(self,
        ...
        if m == "AIC":
            ...
            raise Exception(
                f"Variable m must be either set to 'AIC' or an integer. You set it to {m}.")
        self.mu = mu
        self.p = p
        self.alpha = alpha
    ...
    def boosting_regression(self, X: np.ndarray, indices_of_parents: list[int], idx_target: int, m_stop: int = 100, mu: float = 0.3):
        """
        ...
        for m in range(m_stop):
            ...
            idx_best_parent = indices_of_parents[np.argmin(
                scores_for_potential_parents)]
            chosen_parents[m] = idx_best_parent
            regression_estimate += mu * \
                self.calculate_kernel_regression(u, idx_best_parent)
            # Update u
    ...
    def calculate_gram_matrices(self, X: np.ndarray) -> np.ndarray:
        if self.gram_matrices is None:
            self.gram_matrices = np.zeros(
                (self.p, X.shape[0], X.shape[0]))
        for idx_parent in range(X.shape[1]):
            self.gram_matrices[idx_parent, :, :] = self.kernel_function(
                X[:, idx_parent].reshape(-1, 1), X[:, idx_parent].reshape(-1, 1)) / X.shape[0]
    ...
    def pruning(self, pruning_method, X: np.ndarray) -> list[tuple]:
        ...
        for k in graph_before_pruning.nodes:
            ...
            chosen_parents = np.array(possible_parents)[
                chosen_parents_mask]
            logging.debug(
                f"The chosen parents are {chosen_parents}")
            edges_to_add = [(par, k)
                            for par in chosen_parents]
...
class DAGBooster(BaseDAGBooster):
    ...
    def train(self, X: np.ndarray):
        ...
        for m_curr in range(self.m):
            ...
            delta_estimate_for_idx_target = self.mu * \
                self.calculate_kernel_regression(us[:, idx_target], idx_source)
            self.x_hats[idx_target, idx_source,
                        :] += delta_estimate_for_idx_target
            us[:, idx_target] = old_us_for_idx_target - \
                delta_estimate_for_idx_target
            # Update scores
            self._resids_in_components[idx_target] = np.sum((np.sum(
                self.x_hats[idx_target, :, :], axis=0) - X[:, idx_target]) ** 2)  # np.sum((
            self.sum_squared_residuals_.append(
                np.sum(self._resids_in_components))
            for k in range(self.p):
                if k != idx_target:
            ...
            if self.aic:
                hat_matrices_multiplied[idx_target, :, :] = (np.eye(N) - self.mu * self.hat_matrices[idx_source,
                                                                                                     :, :]) @ hat_matrices_multiplied[idx_target, :, :]

                aic_score_old = aic_score
                traces_individual = N - \
                    np.trace(hat_matrices_multiplied, axis1=1, axis2=2)
                ...
                if aic_score_old < aic_score:
                    print(
                        f"The AIC score went up by {aic_score - aic_score_old}")
                    break
                # If all possible remaining edges are already in graph, then stop.
                if len([tuple(e) for e in np.argwhere(self.scores != np.inf) if (e[0], e[1]) not in self.edges]) == 0:
                    break
        ...
        if self.aic:
            self.aic_scores_individual = np.concatenate(
                [np.reshape(sc, (1, -1)) for sc in self.aic_scores_individual], axis=0).T
        return self.edges

    def calculate_K_plus_lambda_inv(self, X: np.ndarray) -> np.ndarray:
        if self.gram_matrices is None:
            self.calculate_gram_matrices(X)
        self.K_plus_lambda_inv = np.zeros(
    ...
    def calculate_hat_matrices(self, X: np.ndarray) -> np.ndarray:
        ...
        for idx in range(X.shape[1]):
            ...
            hat_matrices[idx, :,
                         :] = self.gram_matrices[idx, :, :] @ K_plus_lambda_inv
        self.hat_matrices = hat_matrices
        return hat_matrices

    def calculate_score_improvement(self, us: np.ndarray, idx_target: int) -> np.ndarray:
        ...
        for k in idx_allowed_parents[0]:
            self.scores[k, idx_target] = np.log(np.sum(
                (u_target - u_hats[k])**2))
        self.scores[:, idx_target] -= \
            np.log(self._resids_in_components[idx_target])
    ...
    def feature_selection_boosting(self, X, indices_of_parents, idx_target, m_stop=100, mu=0.3, threshold=0.02):
        chosen_parents = self.boosting_regression(
            X, indices_of_parents, idx_target, m_stop=m_stop, mu=mu)
        parents_how_often_chosen = [
            (k, v/m_stop) for k, v in dict(collections.Counter(chosen_parents)).items()]
        return [k[0] for k in parents_how_often_chosen if k[1] > threshold]
    ...
    def update_edges_causing_cycle(self, idx_source: int, idx_target: int) -> None:
        ...
        nodes_with_path_to_idx_source = np.where(
            self.path_matrix[:, idx_source] == 1)
        ...
        nodes_with_path_to_idx_target = np.where(
            self.path_matrix[idx_target, :] == 1)
...
class SmallPDAGBooster(BaseDAGBooster):
    ...
    def train(self, X: np.ndarray):
        ...
        for k, permutation in enumerate(all_permutations):
            scores[k] = self.train_permutation(X, permutation)

        idx_min = np.argmin(scores)
        best_permutation = all_permutations[idx_min]
        self.edges = [(best_permutation[k], best_permutation[k+1])
                      for k in range(self.p-1)]
        self.best_permutation = best_permutation
        return self.edges

    def train_permutation(self, X: np.ndarray, permutation: list[int]) -> float:
        # check if X has number of columns p
        assert X.shape[1] == self.p
        N = X.shape[0]
        ...
        for k, idx_target in enumerate(permutation):
            ...
            if k == 0:
                ...
                if self.aic:
                    aic_score = np.inf
                # The gram matrix is the sum of the gram matrices with index small than idx_target
                curr_gram_matrix = np.sum(
                    self.gram_matrices[indices_before_idx_target, :, :], axis=0)
                # calculate hat matrix
                hat_matrix = curr_gram_matrix @ linalg.inv(
                    curr_gram_matrix + self.alpha * np.eye(N))
                for m_curr in range(self.m):
                    us -= self.mu * hat_matrix @ us.copy()
                    if self.aic:
                        if m_curr == 0:
                            prod_identity_minus_hat_matrix = np.eye(
                                N) - self.mu * hat_matrix
                        else:
                            prod_identity_minus_hat_matrix = prod_identity_minus_hat_matrix @ (
                                np.eye(N) - self.mu * hat_matrix)
                        trace_b_m = N - \
                            np.trace(prod_identity_minus_hat_matrix)
