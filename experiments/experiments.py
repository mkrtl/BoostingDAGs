...
def run_experiment(
    ...
    logging.info(f"The true graph has {len(data_generator.graph.edges)} edges.")
    adjacency_matrix_true = nx.adjacency_matrix(data_generator.graph, nodelist=range(p))
    ...
    dag_boost_after_pruning = dag_boost.pruning(gam_pruning, data_generator.data)
    end = time.time()
    duration = end - start
    adjacency_matrix_after_pruning = nx.adjacency_matrix(
        dag_boost_after_pruning, nodelist=range(p)
    )
    shd = hamming_distance(adjacency_matrix_true, adjacency_matrix_after_pruning)

    n_true_positives = len(
        [
            e
            for e in set(dag_boost_after_pruning.edges)
            if e in data_generator.graph.edges
        ]
    )
    ...
    n_false_positives = len(
        [
            e
            for e in set(dag_boost_after_pruning.edges)
            if e not in data_generator.graph.edges
        ]
    )
    ...
    n_false_negatives = len(
        [
            e
            for e in data_generator.graph.edges
            if e not in set(dag_boost_after_pruning.edges)
        ]
    )
    ...
    if store_result_edges:
        root_path_edges = f"{save_path}/graph_{p}_{N}_additive_{additive_structural_equations}_{graph_type}/{seed}"
        if not os.path.exists(root_path_edges):
            os.makedirs(root_path_edges)
        path_edges = f"{root_path_edges}/boosting_cam_edges.csv"      
        nx.write_edgelist(dag_boost_after_pruning, path_edges, delimiter=",", data=False)
    return dict(
        SHD=shd,
        n_true_positives=n_true_positives,
        n_false_positives=n_false_positives,
        n_false_negatives=n_false_negatives,
        n_edges_true_graph=len(data_generator.graph.edges),
        seed=seed,
        duration=duration,
    )
...
def main(
    ...
    logging.info(
        f"We run {n_experiments} experiments with {p} nodes, additive = {additive_structural_equations}, {N} samples, expected number of edges {expected_n_edges} and graph type {graph_type}."
    )
    save_path = f"{path_data}/dens_edges_{expected_n_edges:.2f}/"
    # results:
    shd_boosting = []
    ...
    for seed in run_seeds[:n_exps]:
        ...
        def run_dag_boost_variant_and_evaluate(
            type_of_estimate,
            pns_matrix=np.zeros((p, p), dtype=bool),
            n_edges_stop=(p * (p - 1)) / 2,
        ):
            ...
            if type_of_estimate == "AIC":
                ...
                dag_boost = DAGBooster(
                    ...
                    mu=mu,
                    alpha=alpha,
                    n_edges_stop=n_edges_stop,
                    n_max_parents=n_max_parents,
                )
                dag_boost.train(data_generator.data)
            adjacency_matrix_estimated = dag_boost.get_adjacency_matrix()
            logging.info(f"The estimator has {len(set(dag_boost.edges))} edges.")
            ...
            logging.info(f"The structural hamming distance is {shd_before_pruning}")
            logging.info(
                f"The structural hamming distance after pruning is {shd_after_pruning}"
            )
            curr_number_of_falsely_directed_edges = len(
                [
                    e
                    for e in set(dag_boost_after_pruning.edges)
                    if (e[1], e[0]) in data_generator.graph.edges
                ]
            )
            logging.info(
    ...
    return_dict = {
        "mean_falsely_directed_edges AIC": np.mean(number_of_falsely_directed_edges),
...
def parallel_notears_execution(paths, ncores=None):
    ...
    pool = mp.Pool(ncores)
    with pool:
        processes = [pool.apply_async(main_notears, args=(path,)) for path in paths]
...
if __name__ == "__main__":
    ...
    def target_path(t):
        return f"{root_path_results}/{t}_{N}_{p}_{expected_n_edges}_{graph_type}_{additive}_{n_exps}.csv"
    """
